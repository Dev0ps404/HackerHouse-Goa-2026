...
function renderBuilderFormat(
  ...
  const displayName = safeName ? safeName.toUpperCase() : 'YOUR NAME';
  ctx.save();
  ctx.fillStyle = '#FFFFFF';
  ...
  const displayRole = safeRole ? safeRole.toUpperCase() : 'FULL STACK BUILDER';
  ctx.fillStyle = '#A7F3D0';
  ...
  currentY += 42;
  const safeTitle = typeof builderTitle === 'string' ? builderTitle.trim() : '';
  const displayTitle = safeTitle || 'THE BUILDER';
